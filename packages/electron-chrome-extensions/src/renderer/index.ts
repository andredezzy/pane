...
export const injectExtensionAPIs = () => {
	...
	const invokeExtension = async (
		...
		if (options.noop) {
			...
			if (callback) {
				callback(options.defaultResponse);
			}
	...
	function mainWorldScript() {
		...
		const electron =
			((globalThis as any).electron as typeof electronContext) ||
			electronContext;

		const chrome: any = globalThis.chrome || {};
		...
		class ExtensionEvent<T extends Function> implements chrome.events.Event<T> {
			...
			getRules(_ruleIdentifiers: any, _callback?: any) {
				throw new Error("Method not implemented.");
			}
			hasListener(_callback: T): boolean {
				throw new Error("Method not implemented.");
			}
			removeRules(
			...
			removeRules(_ruleIdentifiers?: any, _callback?: any) {
				throw new Error("Method not implemented.");
			}
			addRules(
				_rules: chrome.events.Rule[],
				_callback?: ((rules: chrome.events.Rule[]) => void) | undefined,
			): void {
				throw new Error("Method not implemented.");
			}
		...
		const browserActionFactory = (
			base: DeepPartial<typeof globalThis.chrome.browserAction>,
		) => {
			const api = {
				...base,
				...
				setBadgeBackgroundColor: invokeExtension(
					"browserAction.setBadgeBackgroundColor",
				),
				getBadgeBackgroundColor: invokeExtension(
		...
		const apiDefinitions: Partial<APIFactoryMap> = {
			action: {
				shouldInject: () =>
					manifest.manifest_version === 3 && !!manifest.action,
				factory: browserActionFactory,
			},

			browserAction: {
				shouldInject: () =>
					manifest.manifest_version === 2 && !!manifest.browser_action,
				factory: browserActionFactory,
			},
			...
			downloads: {
				factory: (base) => {
					return {
						...
						onDeterminingFilename: new ExtensionEvent(
			...
			extension: {
				factory: (base) => {
					return {
						...base,
						isAllowedFileSchemeAccess: invokeExtension(
						...
						isAllowedIncognitoAccess: invokeExtension(
			...
			notifications: {
				factory: (base) => {
					return {
						...
						getPermissionLevel: invokeExtension(
						...
						onButtonClicked: new ExtensionEvent(
			...
			topSites: {
				factory: () => {
					return {
						get: invokeExtension("topSites.get", {
							noop: true,
							defaultResponse: [],
						}),
					};
			...
			webNavigation: {
				factory: (base) => {
					return {
						...
						onBeforeNavigate: new ExtensionEvent(
						...
						onDOMContentLoaded: new ExtensionEvent(
							"webNavigation.onDOMContentLoaded",
						),
						onErrorOccurred: new ExtensionEvent(
							"webNavigation.onErrorOccurred",
						),
						onHistoryStateUpdated: new ExtensionEvent(
			...
			webRequest: {
				factory: (base) => {
					return {
						...base,
						onHeadersReceived: new ExtensionEvent(
