...
ipcRenderer.on("crx-shim-event", (_event, namespace, eventName, ...payload) => {
	if ("executeInMainWorld" in contextBridge) {
		...
		if (d) {
			d(namespace, eventName, payload.length === 1 ? payload[0] : payload);
		}
	}
});
...
if ("executeInMainWorld" in contextBridge) {
	contextBridge.executeInMainWorld({
		func: () => {
			...
			globalThis.__crxEvents = (ns, evt, data) => {
				var key = `${ns}.${evt}`;
				var ls = eventMap[key];
			...
			function makeEvent(ns, eventName) {
				...
				return {
					...
					hasListener: (cb) => ls.indexOf(cb) !== -1,
			...
			if (id !== "eiaeiblijfjekdanodkjadfinkhbfgcd") {
				return;
			}
			...
			for (var i = 0; i < nativeKeys.length; i++) {
				try {
					cached[nativeKeys[i]] = oc[nativeKeys[i]];
				} catch (e) {}
			}
			...
			var extras = {
				...
				alarms: {
					create: (n, i) =>
						ipc ? ipc.invoke("alarms", "create", n, i) : Promise.resolve(),
					get: (n) =>
						ipc ? ipc.invoke("alarms", "get", n) : Promise.resolve(void 0),
					getAll: () =>
						ipc ? ipc.invoke("alarms", "getAll") : Promise.resolve([]),
					clear: (n) =>
						ipc ? ipc.invoke("alarms", "clear", n) : Promise.resolve(true),
					clearAll: () =>
						ipc ? ipc.invoke("alarms", "clearAll") : Promise.resolve(true),
				...
				idle: {
					setDetectionInterval: (s) => {
						if (ipc) {
							ipc.invoke("idle", "setDetectionInterval", s);
						}
					},
					queryState: (s) =>
						ipc
							? ipc.invoke("idle", "queryState", s)
				...
				windows: {
					...
					create: (o) =>
						ipc
							? ipc.invoke("windows", "create", o)
							: Promise.resolve({ id: 1 }),
					get: (id, o) =>
						ipc
							? ipc.invoke("windows", "get", id, o)
							: Promise.resolve({ id: 1 }),
					getCurrent: (o) =>
						ipc
							? ipc.invoke("windows", "getCurrent", o)
							: Promise.resolve({ id: 1 }),
					getLastFocused: (o) =>
						ipc
							? ipc.invoke("windows", "getLastFocused", o)
							: Promise.resolve({ id: 1 }),
					getAll: (o) =>
						ipc ? ipc.invoke("windows", "getAll", o) : Promise.resolve([]),
					update: (id, i) =>
						ipc
							? ipc.invoke("windows", "update", id, i)
							: Promise.resolve({ id: 1 }),
					remove: (id) =>
						ipc ? ipc.invoke("windows", "remove", id) : Promise.resolve(),
					onCreated: noopEvent(),
					onRemoved: noopEvent(),
				...
				privacy: {
					network: {
						...
						webRTCIPHandlingPolicy: {
							get: () => Promise.resolve({ value: "default" }),
							set: noop,
							clear: noop,
						},
					},
			...
			var proxy = new Proxy(
				{},
				{
					...
					set: () => true,
					has: (_t, k) => k in extras || k in cached,
					defineProperty: () => true,
