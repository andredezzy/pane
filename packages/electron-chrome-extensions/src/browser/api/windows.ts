...
export class WindowsAPI {
	...
	private createWindowDetails(win: Electron.BaseWindow) {
		const details: Partial<chrome.windows.Window> = {
			...
			tabs: Array.from(this.ctx.store.tabs)
				...
				.map(
					(tab) =>
						this.ctx.store.tabDetailsCache.get(tab.id) as chrome.tabs.Tab,
				)
				.filter(Boolean),
	...
	private get(_event: ExtensionEvent, windowId: number) {
		const win = this.getWindowFromId(windowId);

		if (!win) {
			return { id: WindowsAPI.WINDOW_ID_NONE };
		}

		return this.getWindowDetails(win);
	}

	private getLastFocused(_event: ExtensionEvent) {
		const win = this.ctx.store.getLastFocusedWindow();

		return win ? this.getWindowDetails(win) : null;
	}

	private getAll(_event: ExtensionEvent) {
	...
	onFocusChanged(windowId: number) {
		if (this.ctx.store.lastFocusedWindowId === windowId) {
			return;
		}
