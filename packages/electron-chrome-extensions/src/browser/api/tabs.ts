...
export class TabsAPI {
	...
	private observeTab(tab: TabContents) {
		...
		const faviconHandler = (_event: Electron.Event, favicons: string[]) => {
	...
	private get(_event: ExtensionEvent, tabId: number) {
		const tab = this.ctx.store.getTabById(tabId);

		if (!tab) {
			return { id: TabsAPI.TAB_ID_NONE };
		}
	...
	private async create(
		...
		const url = details.url
			? validateExtensionUrl(details.url, event.extension)
			: undefined;
	...
	private query(_event: ExtensionEvent, info: chrome.tabs.QueryInfo = {}) {
		...
		const filteredTabs = Array.from(this.ctx.store.tabs)
			...
			.filter((tab) => {
				...
				if (isSet(info.frozen) && info.frozen !== tab.frozen) {
					return false;
				}

				if (isSet(info.groupId) && info.groupId !== tab.groupId) {
					return false;
				}
				...
				if (
					...
					if (!matchesTitlePattern(info.title, tab.title)) {
						return false;
					}
				}
				...
				if (isSet(info.windowId)) {
					if (info.windowId === TabsAPI.WINDOW_ID_CURRENT) {
						if (this.ctx.store.lastFocusedWindowId !== tab.windowId) {
							return false;
						}
	...
	private reload(_event: ExtensionEvent, arg1?: unknown, arg2?: unknown) {
	...
	private async update(event: ExtensionEvent, arg1?: unknown, arg2?: unknown) {
		...
		const url = props.url
			? validateExtensionUrl(props.url, event.extension)
			: undefined;
	...
	private remove(_event: ExtensionEvent, id: number | number[]) {
	...
	private goForward(_event: ExtensionEvent, arg1?: unknown) {
	...
	private goBack(_event: ExtensionEvent, arg1?: unknown) {
		const tabId = typeof arg1 === "number" ? arg1 : undefined;
