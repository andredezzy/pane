...
): chrome.runtime.Manifest => extension.manifest;
...
export const validateExtensionResource = async (
	extension: Electron.Extension,
	uri: string,
) => {
...
export const getIconImage = (extension: Electron.Extension) => {
	...
	const iconAbsolutePath =
		iconPath && resolveExtensionPath(extension, iconPath);

	return iconAbsolutePath
		? nativeImage.createFromPath(iconAbsolutePath)
		: undefined;
};
...
export const matchesPattern = (pattern: string, url: string) => {
	...
	const regexp = new RegExp(
		`^${pattern.split("*").map(escapePattern).join(".*")}$`,
	);
