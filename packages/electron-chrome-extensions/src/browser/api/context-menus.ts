...
const getContextTypesFromParams = (
	params: Electron.ContextMenuParams,
): Set<ContextMenuType> => {
...
const matchesConditions = (
	...
	const inContext = contexts.some((context) =>
		contextTypes.has(context as ContextMenuType),
	);
	...
	if (
		...
		props.targetUrlPatterns.length > 0 &&
		targetUrl
	) {
		if (
			!props.targetUrlPatterns.some((pattern) =>
				matchesPattern(pattern, targetUrl),
			)
		) {
			return false;
		}
	...
	if (
		props.documentUrlPatterns &&
		props.documentUrlPatterns.length > 0 &&
		documentUrl
	) {
		if (
			!props.documentUrlPatterns.some((pattern) =>
				matchesPattern(pattern, documentUrl),
			)
		) {
			return false;
		}
...
export class ContextMenusAPI {
	...
	private buildMenuItem = (opts: ContextItemConstructorOptions) => {
		...
		const menuItemOptions: MenuItemConstructorOptions = {
			...
			label: params
				? formatTitle(props.title || "", params)
				: props.title || "",
			icon,
	...
	private buildMenuItemsFromTemplate = (
		menuItemTemplates: ContextItemConstructorOptions[],
	) => {
	...
	buildMenuItemsForParams(
		...
		if (webContents.session !== this.ctx.session) {
			return [];
		}
		...
		for (const [extensionId, propItems] of this.menus) {
			...
			const topLevelItems = extensionMenuItemOptions.filter(
				(opt) => !opt.props.parentId,
			);
