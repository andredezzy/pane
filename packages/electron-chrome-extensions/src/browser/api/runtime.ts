...
export class RuntimeAPI extends EventEmitter {
	...
	constructor(private ctx: ExtensionContext) {
		...
		handle("runtime.connectNative", this.connectNative, {
			permission: "nativeMessaging",
		});

		handle("runtime.disconnectNative", this.disconnectNative, {
			permission: "nativeMessaging",
		});

		handle("runtime.openOptionsPage", this.openOptionsPage);

		handle("runtime.sendNativeMessage", this.sendNativeMessage, {
			permission: "nativeMessaging",
		});
	}
	...
	private disconnectNative = (_event: ExtensionEvent, connectionId: string) => {
