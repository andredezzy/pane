...
const shortenValues = (_k: string, v: any) =>
...
class RoutingDelegate {
	...
	addObserver(observer: RoutingDelegateObserver) {
		...
		const maybeListenForWorkerEvents = ({
			...
			if (
				...
				d(
					`listening to service worker [versionId:${versionId}, scope:${serviceWorker.scope}]`,
				);
		...
		observer.session.serviceWorkers.on(
			"running-status-changed",
			maybeListenForWorkerEvents,
		);
	}
	...
	private onRouterMessage = async (
		...
		return observer?.onExtensionMessage(
			event,
			extensionId,
			handlerName,
			...args,
...
export class ExtensionRouter {
	...
	constructor(
		...
		app.on("web-contents-created", (_event, webContents) => {
			if (
				...
				d(
					`storing reference to background host [url:'${webContents.getURL()}']`,
				);
	...
	private observeListenerHost(host: FrameEventListener["host"]) {
		...
		host.once("destroyed", () => {
			...
			this.filterListeners(
				(listener) => listener.type !== "frame" || listener.host !== host,
			);
		});
	}
	...
	addListener(listener: EventListener, extensionId: string, eventName: string) {
		...
		if (!extension) {
			throw new Error(
				`extension not registered in session [extensionId:${extensionId}]`,
			);
		}
		...
		const existingEventListener = eventListeners.find(
			eventListenerEquals(listener),
		);
	...
	async onExtensionMessage(
		...
		if (eventSession !== session && !handler.allowRemote) {
			throw new Error(
				`${handlerName} does not support calling from a remote session`,
			);
		}

		const extension = extensionId
			? eventSessionExtensions.getExtension(extensionId)
			: undefined;

		if (!extension && handler.extensionContext) {
			throw new Error(
				`${handlerName} was sent from an unknown extension context`,
			);
		}
	...
	private handle(
		name: string,
		callback: HandlerCallback,
		opts?: Partial<HandlerOptions>,
	): void {
		this.handlers.set(name, {
			callback,
			extensionContext:
				typeof opts?.extensionContext === "boolean"
					? opts.extensionContext
					: true,
			allowRemote:
				typeof opts?.allowRemote === "boolean" ? opts.allowRemote : false,
			permission:
				typeof opts?.permission === "string" ? opts.permission : undefined,
		});
	}
